...
```{r nrow}
iris %>% summarise(n = n()) # returns a tibble

#nrow(iris) # returns a vector with one integer
```
...
```
### Numbered bullets
we just showed data split into bins by
...
iris %>% summarize(var(Sepal.Width))
...
iris$Species %>% table()  %>% prop.table() %>% round(digits=3)
```
```{r Exploration of factor variables 5.5}
iris %>%
  group_by(Species) %>%
  summarize(n = n()) %>%
  mutate(prop = n/sum(n))
...
```{r pairs tidy}
iris %>% select(-Species) %>% pairs.panels()
...
#cor(iris[,c(2,1)])
```
```{r correlation specific columns in tidy and base R using purr}
iris %>% 
  group_by(Species) %>%
  summarize(cor=cor(Sepal.Width,Sepal.Length))
```
...
```{r covariance specific columns in tidy and base R by species}
iris %>% 
  group_by(Species) %>%
  summarize(cov = cov(Sepal.Width,Sepal.Length)) 
